...
- When Highlight panels and Print Panel Numbers are both enabled, render every number in the GUI preview at full opacity and the Large 28 px visible size. Treat this strictly as preview emphasis: do not change the selected number-size preset, derived export size, export opacity, manual anchor, or any exported artifact.
