...
- For Auto paneling background detection, flood-fill background-like pixels from the low-resolution mask edges using four-way connectivity. Treat only edge-connected background as empty; retain opaque background-colored regions enclosed by artwork so white interiors are not omitted. Transparent enclosed pixels remain empty.
- Place a **Preserve enclosed artwork** toggle inside a dedicated **Advanced** subsection within the expanded Auto paneling options. Make Advanced an accessible, collapsed-by-default disclosure with a short slide/fade animation and synchronized `aria-expanded`, `aria-hidden`, and disabled state. Enable Preserve enclosed artwork by default and persist it in browser local storage. Recalculate an active automatic layout immediately when it changes. When disabled, use the original independent per-pixel background comparison instead of edge-connected flood filling.
...
- `Shift+F` toggles **Floating preview**, `Shift+H` toggles **Highlight panels**, `Shift+N` toggles **Print Numbers**, `Shift+O` starts or finishes assembly-order editing, and `Ctrl+Enter` on Windows/Linux or `Cmd+Enter` on macOS starts ZIP export. Keep every shortcut active while a range slider has focus. Ignore Shift-based action shortcuts in other editable controls such as number fields, selects, and text inputs. Keep the modified-Enter export shortcut active from all settings controls. Ignore every action shortcut while the export modal is open, and never invoke a disabled action.
- The **Print Numbers** GUI control is a switch-style toggle directly below **Auto paneling**. Enabled numbers are zero-based plain text with a white outline, use the selected visible-size preset and grid-line color, and derive the corresponding export-pixel font size from the output-to-displayed-preview scale so preview and export have the same relative artwork size in uniform and automatic layouts.
- Add a persisted **Smart Number Placement** toggle inside an animated, collapsible **Advance** subgroup under Panel Labels. Keep the subgroup collapsed by default while leaving the toggle enabled by default and active when hidden. Build a low-resolution distance map that treats exterior background, panel boundaries, dark stroke pixels, and strong color/alpha transitions as unsafe, then choose the highest-clearance occupied point in each panel. For label placement, preserve enclosed light artwork independently of the auto-paneling **Preserve enclosed artwork** toggle so line art retains stroke-free candidates. Exclude unsafe pixels from candidacy instead of merely lowering their score. Fall back to the panel center when no stroke-free occupied candidate exists. A manually dragged number is a per-panel override and must not be replaced by later toggle changes; reset overrides when a new image or panel layout is loaded.
...
- Paper size, unit-system selection, Poster height and Grid width in millimeters, the Auto paneling toggle with its maximum and minimum sides and enclosed-light-area behavior, Floating preview, Highlight panels, the Print Numbers toggle, Smart Number Placement as `smartNumberPlacement`, and the number-size preset persist in browser local storage under ronyka-panel-splitter.display-settings.v1. Do not migrate the former clear-area placement property. Restore the Auto paneling preference immediately, then generate its artwork-aware layout after an image loads. Other GUI settings, manually overridden number positions, and uploaded image data remain transient.
...
- Provide a **Highlight panels** toggle in the Preview settings group, default it off, and persist it in browser local storage. When enabled, draw a preview-only translucent green fill and dark-green frame over every panel using the Assembly Order selection visual language at a lighter intensity. Keep already-selected Assembly Order panels visually stronger, and never include either highlight in exported artwork.
- When Highlight panels and Print Numbers are both enabled, render every number in the GUI preview at full opacity and the Large 28 px visible size. Treat this strictly as preview emphasis: do not change the selected number-size preset, derived export size, export opacity, manual anchor, or any exported artifact.
