...
- Paper size, unit-system selection, Poster height and Grid width in millimeters, the Auto paneling toggle with its maximum and minimum sides, Floating preview, Highlight panels, the Print Panel Numbers toggle, and its size preset persist in browser local storage under ronyka-panel-splitter.display-settings.v1. Restore the Auto paneling preference immediately, then generate its artwork-aware layout after an image loads. Other GUI settings, number positions, and uploaded image data remain transient.
