...
Project summary:
- Current application version: 1.29.0. Keep package.json, package-lock.json, the GUI version label, and CHANGES.md synchronized only when the release is committed.
...
- Paper size, unit-system selection, the Auto paneling toggle, the Print Panel Numbers toggle, and its size preset persist in browser local storage under ronyka-panel-splitter.display-settings.v1. Restore the Auto paneling preference immediately, then generate its artwork-aware layout after an image loads. Other GUI settings, number positions, and uploaded image data remain transient.
