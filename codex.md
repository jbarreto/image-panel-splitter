...
- `Shift+N` toggles **Print Panel Numbers**, `Shift+O` starts or finishes assembly-order editing, and `Ctrl+Enter` on Windows/Linux or `Cmd+Enter` on macOS starts ZIP export. Ignore action shortcuts while focus is in an editable control or the export modal is open, and never invoke a disabled action.
